...
Set fso = CreateObject("Scripting.FileSystemObject")
   ...
   n = 0 
	For x = 0 To 6
		...
		Set file = fso.OpenTextFile( path & "\work\last_month\prod_pd0" & x & ".sql", 1)
		...
		If sql_sv.eof = False Then 
			...
			sql_sv.MoveFirst	
			Set file = fso.OpenTextFile( path & "\work\last_month\" & insert_file & ".sql", 1 )
			content = file.ReadAll
			...
			'sql_sv.MoveNext
			 Next			
				...
				PROGRESS = 100
				Stdout.Write Chr(32) & "PD0" & x & "    Complete " & Chr(33) & Chr(32) & "Record data " & FormatNumber(itt,0) &  String( 6 - Len( FormatNumber(itt,0) ) ," ") & " record " & vbLf 
				n = 0
