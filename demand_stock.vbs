...
Dim StdOut : Set StdOut = CreateObject("Scripting.FileSystemObject").GetStandardStream(2) 
    path = "G:\vbs_demand"
	filename = "demand_stock"
	namelog = "Dlog_" & Year(date) & WhatEver(Month(date)) & WhatEver(Day(date))
	table = "DEMAND_STOCK"
	data_file =   "demand_stock"
	insert_file = "demand_stock_ins"
...
Set fso = CreateObject("Scripting.FileSystemObject")
	Stdout.WriteLine " START   TASK" & Chr(32) & 	Date() & " " & FormatDateTime(Now(),vbLongTime)
		...
		If sql_sv.eof = False Then 
			...
			'sql_sv.MoveNext
			 Next			
				...
				PROGRESS = 100
				Stdout.Write Chr(32) & "Stock" & "   Complete " & Chr(33) & Chr(32) & "Record data " & FormatNumber(itt,0) &  String( 6 - Len( FormatNumber(itt,0) ) ," ") & " record " & vbLf 
				n = 0
